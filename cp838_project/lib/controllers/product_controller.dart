...
import 'package:cp838_project/models/category_model.dart';
...
class ProductController extends GetxController {
  ...
  getRating(data, context) async {
    var rating = 0.0;
    var rat = 0.0;
    var count = 0.0;
    await firestore
        .collection(previewCollection)
        .where('p_id', isEqualTo: data.id)
        .get()
        .then((value) {
      for (var item in value.docs) {
        rat += double.parse(item.data()['p_rating'].toString());
        count = double.parse(value.docs.length.toString());
        print('${item.id} => ${item.data()['p_rating']}');
      }
      print("rating: $rating");
    });
    rating = rat / count;
    await firestore
        .collection(productCollection)
        .doc(data['p_id'])
        .update({'p_rating': rating.toDouble().toString()}).then(
            (value) => VxToast.show(context, msg: 'Rating update'));
  }
  ...
  updatePreview(data, context) async {
    ...
    await store.set({
      ...
      rating = rat / count;
      print("rating: ${rating.floor()}");

      await firestore
          .collection(productCollection)
          .doc(data.id)
          .update({'p_rating': rating.floor().toString()}).then(
              (value) => VxToast.show(context, msg: 'Rating update'));
